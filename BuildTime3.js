...
/* 1) creare funzione che controlla tutti gli object all'interno dell'array;
  3) con un IF, controllare i campi location e title - cambiarli in tutto minuscolo o tutto maiuscolo;
  2) da tutti i risultati trovati, eliminare description, requirements, benefits, company_profile - con ciclo FOR
  4) fare un ciclo for per pushare i risultati all'interno dell'array che siano compatibili
  5) fare un RETURN dei risultati
  6) console.log inserendo i campi "DEV" e "US" */


  function findJob (jobs, position, location) {    //questi 3 sono PARAMS - PARAMETRI//
    let result = []
    for (let job of jobs) {
      if (job.location.toLowerCase().includes(location.toLowerCase())&&(job.title.toLowerCase().includes(position.toLowerCase() .includes(position.toLowerCase())))) {
  ...
  console.log(findJob(jobs, "dev", "us")) 
